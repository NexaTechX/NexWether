...
const API = "7a5a40542c7c9a76cd6bdbdd90b43e62"; // Consider securing this key
...
// Check if the user has a stored location
window.addEventListener("load", () => {
    const storedLocation = localStorage.getItem("lastLocation");

    if (storedLocation) {
        findLocation(storedLocation);
    } else {
...
btnEl.addEventListener("click", (e) => {
    ...
    if (search) {
        localStorage.setItem("lastLocation", search);
        inputEl.value = "";
        findLocation(search);
    } else {
...
async function findLocation(name) {
    ...
    try {
        ...
        if (data.cod !== 200) {
            displayErrorMessage(data.cod === "404" ? "No location found. Please try again." : "Error fetching data. Please try again.");
            return;
        }
